    ...
    switch ($_GET['act']) {
        ...
        case 'deletecart':
            if (isset($_SESSION['giohang']) && count($_SESSION['giohang']) > 0) {
                ...
                if (isset($_GET['idcart'])) {
                    ...
                    echo '<div class="delete-cart-item-success d-none" style="">HELLO</div>';
        ...
        case 'loginpage':
            if (isset($_POST['loginbtn']) && $_POST['loginbtn']) {
                ...
                if ($islogined === -1) {
                    ...
                    echo '<div class="login-account-fail d-none" style="">HELLO</div>';
        ...
        case 'resetpassword':
            if (isset($_POST['resetbtn']) && $_POST['resetbtn']) {
                ...
                $user = findIdbyUser($username);
                // var_dump($user);
                if ($user) {
                    $iduser = $user['id'];
                    $is_updated = updatepass($iduser, $password);
                    if ($is_updated) {
                        echo '<div class="reset-password-success d-none" style="">HELLO</div>';
                    }
                    // header('location: index.php?act=loginpage');
                    include "./view/login-page.php";

                } else {
                    echo '<div class="reset-password-fail d-none" style="">HELLO</div>';
        ...
        case 'signuppage':
            if (isset($_POST['signupbtn']) && $_POST['signupbtn']) {
                ...
                $password = $_POST['password'];
                // connectdb();
                $is_inserted = insertuser($fullname, $homeaddress, $phonenumber, $username, $email, $password);
                // if ($is_inserted) {
